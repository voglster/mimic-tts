...
# below). It never touches any pre-existing key or voice.
set -euo pipefail
...
RESET=$'\033[0m'; RED=$'\033[31m'; GREEN=$'\033[32m'; BOLD=$'\033[1m'
...
cleanup() {
  ...
  # dies partway through never leaves scratch state on a real server.
  set +e
  ...
  if [[ "$ADMIN_VOICE_REGISTERED" == "1" ]]; then
    curl -s -o /dev/null -X DELETE \
      -H "Authorization: Bearer ${MIMIC_ADMIN_TOKEN}" \
      "${URL}/clone/voices/${ADMIN_VOICE_NAME}"
  fi
  rm -rf "$WORKDIR"
}
trap cleanup EXIT
...
expect_status() {
  ...
  if [[ "$status" != "$expected" ]]; then
    fail "$method $path -> $status (expected $expected). Body: $(cat "${WORKDIR}/last_body")"
  fi
  echo "$status"
}
